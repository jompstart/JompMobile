...
const Dashboard = () => {
  ...
  const fetchUnreadCount = useCallback(async () => {
    try {
      ...
      const unreadCountData = unreadResponse.data !== undefined ? unreadResponse.data : unreadResponse;
  ...
  // Refetch unread count when the screen comes into focus
  useFocusEffect(
    useCallback(() => {
      if (user.userId && user.customerId) {
        fetchUnreadCount();
        ...
        refreshControl={
          <RefreshControl
            ...
            onRefresh={() => {
              ...
              reloadRecentTransaction();
              refetchPendingService();
              fetchUnreadCount(); // Add this to refresh unread count on pull-to-refresh
            }}
          />
        }
        showsVerticalScrollIndicator={false}
