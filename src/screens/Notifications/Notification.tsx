...
const Notification: React.FC<NotificationScreenProps> = ({
  ...
  useEffect(() => {
    const fetchNotifications = async () => {
      ...
      try {
        ...
        console.log(
          "Raw API Response for notifications:",
        ...
        console.log(
          "Raw API Response for unread count:",
          JSON.stringify(unreadResponse, null, 2)
        );
  ...
  const handleNotificationPress = async (
    ...
    if (notification.hasRedDot) {
      try {
        const response = await userService.markNotificationRead(
          notification.id
        );
        ...
        const status =
          (response as API_RESPONSE<{ status: string }>).data?.status ??
          response.status;
  ...
  const handleDeleteNotification = async (notificationId: string) => {
    ...
    setState((prev) => ({
      ...
      selectedNotification:
        prev.selectedNotification?.id === notificationId
          ? null
          : prev.selectedNotification,
    ...
    try {
      ...
      setState((prev) => ({
        ...
        notifications: notificationToDelete
          ? [...prev.notifications, notificationToDelete].sort((a, b) =>
          ...
          {state.notifications.length > 0 && (
            ...
            >
              <CText
                ...
                fontFamily="semibold"
                color={
                  state.deletingNotificationId === "all" ? "#ccc" : "#F44336"
                }
              >
                {state.deletingNotificationId === "all"
                  ? "Clearing..."
        ...
        <View style={styles.content}>
          <View style={styles.detailHeader}>
            ...
            <TouchableOpacity
              style={styles.detailBackButton}
              onPress={() =>
                handleDeleteNotification(state.selectedNotification!.id)
              }
              disabled={
                state.deletingNotificationId === state.selectedNotification!.id
              }
            >
              <MaterialIcons
                name="delete"
