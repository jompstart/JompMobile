...
type NavigationProp = StackNavigationProp<RootStackParamList, 'ServiceDetailScreen'>;
...
const ServiceDetailScreen: React.FC<ServiceDetailScreenProps> = ({
  ...
  const deduplicatedPaymentTerms = paymentTerms.reduce<PaymentTerm[]>((acc, term) => {
  ...
  const renderDynamicData = (data: any, depth: number = 0, parentKey: string = ''): JSX.Element[] => {
    ...
    Object.entries(data).forEach(([key, value]) => {
      ...
      if (['id', 'success', 'message', 'statusCode', 'createdAt', 'updatedAt'].includes(key) || 
          value === null || 
          value === undefined ||
          value === 'undefined') {
        return;
      ...
      if (isObject(value) && !Array.isArray(value)) {
        ...
        elements.push(
          <View key={fullKey} style={[styles.section, { marginLeft: depth * 10 }]}>
            <CText
              color={'primaryColor'}
              fontSize={18 - depth}
              lineHeight={24 - depth}
        ...
        elements.push(
          <View key={fullKey} style={styles.detailItem}>
            <CText color={'black'} fontSize={16} lineHeight={22} fontFamily="bold">
              {formattedKey}:
            </CText>
            <View style={styles.arrayContainer}>
              {value.map((item, index) => (
                <CText key={index} color={'secondaryBlack'} fontSize={14} lineHeight={18} fontFamily="regular">
        ...
        } else if (isNumeric(value) && (key.toLowerCase().includes('amount') || key.toLowerCase().includes('fee') || key.toLowerCase().includes('price'))) {
          ...
          <View key={fullKey} style={styles.detailItem}>
            ...
            {isLink ? (
              <Pressable onPress={() => openDocument(value as string)}>
                <CText color={'primaryColor'} fontSize={16} lineHeight={22} fontFamily="regular" style={styles.link}>
                  📄 {displayValue}
                </CText>
              </Pressable>
            ) : (
  ...
  useEffect(() => {
    const fetchDetails = async () => {
      ...
      setError(null);
      try {
        console.log(`Fetching details for serviceId: ${service.id}, serviceType: ${serviceType}`);
  ...
  const retryFetch = () => {
    ...
    const fetchDetails = async () => {
      try {
        console.log(`Retrying fetch for serviceId: ${service.id}, serviceType: ${serviceType}`);
        const [detailsResponse, termsResponse, feeResponse] = await Promise.all([
          makeRequest<any>({
            method: 'GET',
            url: `/get-services?serviceId=${service.id}&serviceType=${serviceType}`,
        ...
        console.log('Service details (retry):', JSON.stringify(detailsResponse, null, 2));
  ...
  const calculateTotalFees = () => {
    ...
    return processingFee.processingFee + processingFee.insuranceFee + processingFee.adminFee;
  };
          ...
          <>
            {error && (
              <View style={styles.errorContainer}>
                ...
                <Pressable
                  style={[styles.primaryButton, { marginTop: size.getHeightSize(10) }]}
                  onPress={retryFetch}
                >
                  <CText
                    color={'white'}
            ...
            <View style={styles.section}>
              ...
              <View style={styles.detailItem}>
                ...
                <CText color={'secondaryBlack'} fontSize={16} lineHeight={22} fontFamily="regular">
                  {service.name}
                </CText>
              </View>
              <View style={styles.detailItem}>
              ...
              <View style={styles.detailItem}>
                ...
                <CText color={'primaryColor'} fontSize={16} lineHeight={22} fontFamily="bold">
                  {service.currencyCode || 'NGN'} {formatToAmount(service.price)}
                </CText>
              </View>
              {service.status && (
                <View style={styles.detailItem}>
                  ...
                  <View style={[
                    styles.statusBadge,
                    {
                      backgroundColor: service.status in mappedStatus 
                        ? `${mappedStatus[service.status].color}20` 
                        : '#00000020'
                    }
                  ]}>
                    <CText
                      color={service.status in mappedStatus ? mappedStatus[service.status].color : 'black'}
                      fontSize={14}
                      lineHeight={18}
                      fontFamily="regular"
                    >
                      {service.status in mappedStatus ? mappedStatus[service.status].label : service.status}
                    </CText>
                  </View>
                </View>
              )}
                  ...
                  {deduplicatedPaymentTerms
                    ...
                    .map((term) => (
                      <View key={term.id} style={styles.paymentTermItem}>
                        <CText color={'secondaryBlack'} fontSize={14} lineHeight={18} fontFamily="regular">
                          {term.name}
                        </CText>
                      </View>
                    ))}
