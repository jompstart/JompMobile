...
const AddBank = () => {
  ...
  const [bankData, setBankData] = useState<Array<Banks>>([]);
  const [filteredBankData, setFilteredBankData] = useState<Array<Banks>>([]);
  const { data, isLoading, isError: isBanksError } = useGetBanks(user.userId, user.customerId);
  const [showBankList, setShowBankList] = useState(false);
  const [selectedBank, setSelectedBank] = useState<Banks | null>(null);
  const [accountNumber, setAccountNumber] = useState('');
  ...
  const {
    ...
    onError: (error: any) => {
      ...
      dispatch(
        updateToast({
          toastMessage: error?.message || 'Unable to verify account number. Please check and try again.',
          displayToast: true,
          toastType: 'info',
        })
      );
    ...
    onSuccess: (data) => {
      dispatch(
        updateToast({
          toastMessage: `Account verified: ${data?.data?.account_name}`,
          displayToast: true,
          toastType: 'success',
        })
      );
  ...
  >({
    ...
    onError: (error: any) => {
      ...
      dispatch(
        updateToast({
          toastMessage: error?.message || 'Failed to add bank account. Please try again.',
          displayToast: true,
          toastType: 'info',
        })
      );
    ...
    onSuccess: (data) => {
      dispatch(
        updateToast({
          toastMessage: 'Bank account added successfully!',
          displayToast: true,
          toastType: 'success',
        })
      );
  ...
  useEffect(() => {
    if (isBanksError) {
      dispatch(
        updateToast({
          toastMessage: 'Unable to load banks. Please check your connection.',
          displayToast: true,
          toastType: 'info',
        })
      );
  ...
  const handleSubmit = () => {
    if (!selectedBank) {
      dispatch(
        updateToast({
          toastMessage: 'Please select a bank',
          displayToast: true,
          toastType: 'info',
        })
      );
    ...
    if (accountNumber.length !== 10) {
      dispatch(
        updateToast({
          toastMessage: 'Please enter a valid 10-digit account number',
          displayToast: true,
          toastType: 'info',
        })
      );
    ...
    if (!validationSuccess) {
      dispatch(
        updateToast({
          toastMessage: 'Please wait for account verification to complete',
          displayToast: true,
          toastType: 'info',
        })
      );
    ...
    <GradientSafeAreaView>
      ...
      <ScrollView>
          ...
          <Pressable
            onPress={() => {
              if (isLoading) {
                dispatch(
                  updateToast({
                    toastMessage: 'Loading banks, please wait...',
                    displayToast: true,
                    toastType: 'info',
                  })
                );
        ...
        >
          ...
          <PrimaryButton
            ...
            isLoading={isAddingBank}
            disabled={!validationSuccess || isAddingBank}
            style={{
              marginTop: size.getHeightSize(40),
            }}
            label="Submit"
      ...
      <BanksBottomsheet
        ...
        onSelectedBank={(bank) => {
          ...
          dispatch(
            updateToast({
              toastMessage: `${bank.name} selected`,
              displayToast: true,
              toastType: 'success',
            })
          );
