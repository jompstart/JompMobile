...
export class ProviderService {
  ...
  async registerSchoolFeeForOthers(data: ChildSchoolFeeRequest) {
    ...
    data.childDetails.forEach((child, index) => {
      formData.append(`ChildDetails[${index}].ChildFirstName`, child.childFirstName);
      formData.append(`ChildDetails[${index}].ChildLastName`, child.childLastName);
      formData.append(`ChildDetails[${index}].SchoolEmail`, child.schoolEmail);
      formData.append(`ChildDetails[${index}].SchoolFee`, child.schoolFee.toString());
      formData.append(`ChildDetails[${index}].SchoolName`, child.schoolName);
      formData.append(
        `ChildDetails[${index}].SchoolLocation`,
        `${child.schoolAddress} ${child.schoolAddress2} ${child.schoolLocation} ${child.postalCode || ''} ${child.country}`
      );
      formData.append(`ChildDetails[${index}].ChildGrade`, child.childGrade);
      formData.append(`ChildDetails[${index}].invoice`, child.invoice as any);
    });
    ...
    formData.append('ParentRecords.CompanyEmail', data.workDetails.email);
    formData.append('ParentRecords.CompanyPhoneNumber', this.formatPhoneNumber(data.workDetails.phone));
    formData.append('ParentRecords.UtilityBill', data.documentUploads.utilityBill as any);
    formData.append('ParentRecords.PaySlip', data.documentUploads.paySlip as any);
    formData.append('ParentRecords.BankStatement', data.documentUploads.bankStatement as any);
    if (data.documentUploads.bankStatement2) {
      formData.append('ParentRecords.BankStatement2', data.documentUploads.bankStatement2 as any);
    }
    formData.append('isMobile', `${true}`);
    formData.append('IdempotencyKey', data.IdempotencyKey);
  ...
  async registerSchoolFee(data: SelfSchoolFeeDetails) {
    ...
    formData.append('CustomerId', this.customerId);
    formData.append('UtilityBill', data.documentUploads.utilityBill as any);
    formData.append('EducationDetailRequest.CourseOfStudy', data.educationnDetails.course!);
    formData.append('EducationDetailRequest.EducationLevel', data.educationnDetails.level!);
    formData.append('EducationDetailRequest.InstitutionName', data.educationnDetails.nameOfSchool!);
    formData.append(
      'EducationDetailRequest.InstitutionAddress',
      `${data.educationnDetails.location! || data.educationnDetails.locationOfSchool2} ${data.educationnDetails.city!} ${data.educationnDetails.state!} ${data.educationnDetails.postalCode!} ${data.educationnDetails.country!}`
    );
    formData.append('EducationDetailRequest.Zip', data.educationnDetails.postalCode!);
    formData.append('EducationDetailRequest.TuitionFee', data.educationnDetails.tuitionFee!);
    formData.append('EducationDetailRequest.LoanAmount', data.educationnDetails.loanAmount!);
    formData.append('EducationDetailRequest.TuitionFeeInvoice', data.documentUploads.schoolFeeInvoice as any);
    formData.append('EducationDetailRequest.OfficialSchoolId', data.documentUploads.schoolIdCard as any);
    formData.append('EmploymentIncomeInformationRequest.Occupation', data.employmentDetails.occupation!);
    formData.append('EmploymentIncomeInformationRequest.EmployerName', data.employmentDetails.employerName!);
    formData.append(
      'EmploymentIncomeInformationRequest.EmploymentDuration',
      `${data.employmentDetails.yearsInCompany!}years ${data.employmentDetails.month!}months`
    );
    formData.append('EmploymentIncomeInformationRequest.OfficialWorkEmail', data.employmentDetails.companyEmail!);
    formData.append(
      'EmploymentIncomeInformationRequest.HRCompanyContactNumber',
      this.formatPhoneNumber(data.employmentDetails.hrContactNumber!)
    );
    formData.append(
      'EmploymentIncomeInformationRequest.EmployerContactAddress',
      `${data.employmentDetails.employerAddress!} ${data.employmentDetails.employerCity!} ${data.employmentDetails.employerState!} ${data.employmentDetails.employerPostalCode!} ${data.employmentDetails.employerCountry!}`
    );
    formData.append('EmploymentIncomeInformationRequest.MonthsPayslip', data.employmentDetails.paymentSlip as any);
    formData.append('EmploymentIncomeInformationRequest.MonthsBankStatement', data.documentUploads.bankStatement as any);
    formData.append('AgreedToTermsAndConditions', `${true}`);
    formData.append('AgreedToLoanAgreement', `${true}`);
    formData.append('isMobile', `${true}`);
    formData.append('IdempotencyKey', data.IdempotencyKey);
  ...
  async requestHouseRentLoan(data: HouseRentLoanFormState) {
    ...
    formData.append('LandLordRequest.LandLordName', data.landlordName);
    formData.append('LandLordRequest.AccountName', data.landlordAccountName);
    formData.append('LandLordRequest.AccountNumber', data.landlordAccountNumber);
    formData.append('LandLordRequest.BankName', data.landlordBankName);
    formData.append('LandLordRequest.ContactNumber', data.landlordContactNumber);
    formData.append('RentEmployeerRequest.Occupaction', data.occupation);
    formData.append('RentEmployeerRequest.CompanyName', data.companyName);
    formData.append('RentEmployeerRequest.CompanyPhone', data.companyPhone);
    formData.append('RentEmployeerRequest.YearsOfWorkinWithThem', data.yearsInCompany);
    formData.append('RentEmployeerRequest.CompanyEmail', data.companyEmail);
    formData.append('RentEmployeerRequest.CompanyPhoneNumber', data.companyPhone);
    formData.append('RentEmployeerRequest.CompanyLocation', data.companyAddress);
    formData.append('isMobile', `${true}`);
    formData.append('IdempotencyKey', data.IdempotencyKey);
  ...
  async transportloan(data: TransportRequest) {
    const formData = new FormData();
    formData.append('CustomerId', this.customerId);
    formData.append('TransporterOccupationRequest.EmploymentStatus', data.employmentStatus);
    formData.append('TransporterOccupationRequest.Occupation', data.occupation);
    formData.append('TransporterOccupationRequest.OccupationAddress', data.occupationAddress);
    formData.append('TransporterOccupationRequest.Income', data.income);
    formData.append('TransporterOccupationRequest.Payday', data.payday);
    formData.append('TransporterOccupationRequest.ModeOfPayment', data.modeOfPayment);
    formData.append('TransporterOccupationRequest.EmployerName', data.employerName);
    formData.append('TransporterOccupationRequest.EmployerContactNumber', data.employerContactNumber);
    formData.append('TransportCreditRequest.TransportMode', data.transportMode);
    formData.append('TransportCreditRequest.TransportCost', data.transportCost);
    formData.append('TransportCreditRequest.CreditAmount', data.creditAmount);
    formData.append('TransportCreditRequest.PaymentDuration', data.paymentDuration);
    formData.append('TransportCreditRequest.ProofEmployment', data.proofEmployment as any);
    formData.append('TransportCreditRequest.ValidId', data.validId as any);
    formData.append('TransportCreditRequest.PaySlip', data.paySlip as any);
    formData.append('TransportCreditRequest.BankStatement', data.bankStatement as any);
    formData.append('TransportCreditRequest.Utility', data.utility as any);
    formData.append('TransportCreditRequest.InformationAgreement', `${true}`);
    formData.append('TransportCreditRequest.AuthorizeDebit', `${true}`);
    formData.append('TransportCreditRequest.TermsAndConditions', `${true}`);
  ...
  async requestOtherService(data: Omit<OtherBillsDto, 'CustomerId'>) {
    ...
    formData.append('ServiceProvider', data.ServiceProvider);
    formData.append('GuarantorAddress', data.GuarantorAddress);
    formData.append('GuarantorPhoneNumber', this.formatPhoneNumber(data.GuarantorPhoneNumber));
    formData.append('CostOfService', data.CostOfService);
    formData.append('ServiceCategory', data.ServiceCategory);
    formData.append('ReasonForLoan', data.ReasonForLoan);
    formData.append('GuarantorName', data.GuarantorName);
  ...
  async makePayment(data: MakePaymentDto) {
    return await makeRequest<MakePaymnetApiResponse>({
      ...
      data: {
        customerID: this.customerId,
        amount: data.amount,
        loanAggrement: data.loanAgreement, // Match API spec spelling
      },
    });
  }
  ...
  async verifyPayment(reference: string) {
    return await makeRequest<VerifyPaymentResponse>({
      method: 'GET',
      url: `/down-payment-call-backUrl?reference=${reference}`,
    });
  }
}